...
int main(int argc, char *argv[]){
  ...
  for(i = 0; i<argc; i++){
    ...
    if(LOAD_FILE){
      load_game(dungeon_map, hardness_map, num_rooms, num_up, num_down, num_stairsplaced, rooms, room_array, up_stairs, down_stairs, &player, 1, 0);
...
void create_dungeon_map(char dungeon[MAP_Y_MAX][MAP_X_MAX], uint8_t hardness[MAP_Y_MAX][MAP_X_MAX], int num_rooms, int num_up, int num_down, int num_stairs_placed[2], int rooms[][2], room_t room_array[], up_t up_stairs[], down_t down_stairs[], player_t *player, int save, int bool) {
