...
int is_character(dungeon_t *dungeon, mon_t mons[], int next_pos[2]){
    ...
    for(i = 0; i < dungeon->num_mons + 1; i++){
        if((next_pos[0] == dungeon->mons[i - 1].y_pos && next_pos[1] == dungeon->mons[i - 1].x_pos)){ //If the next space is a monster, kill the monster
            mons[i - 1].alive = 0;
