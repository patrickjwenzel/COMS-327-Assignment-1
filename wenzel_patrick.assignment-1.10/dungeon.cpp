...
void parse_descriptions(Dungeon *dungeon){
    ...
    path2 += "/.rlg327/descriptions/object_desc.txt";
...
void create_monsters(Dungeon *dungeon){
    ...
    for(i = 0; i < int (dungeon->num_mons); i++){ //boob
        placed = 0;
        while(!placed){
            rand_num = 20;//rand() % dungeon->descriptions.monster_list.size();boob
...
int is_monster_or_item(Dungeon *dungeon, int x, int y){
    ...
    for(i = 0; i < (int) dungeon->num_items; i++){
        if((x != dungeon->player.x_pos && y != dungeon->player.y_pos) && (x == dungeon->items[i].x_pos && y == dungeon->items[i].y_pos) && (!dungeon->fow || item_visible(dungeon, dungeon->items[i], dungeon->player))){
            return ((-1 * i) - 1);
        }
    }
    return -999;
